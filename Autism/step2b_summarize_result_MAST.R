...
pdf("figures/step2b_pval_MAST_hist.pdf", width=9, height=9)
par(mfrow=c(3,3), bty="n", mar=c(5,4,3,1))

for(i in 1:length(cell_types)){
  ct1 = cell_types[i]
  p1  = fread(sprintf("res/step1b_MAST_%s_glm.tsv", ct1))
  p2  = fread(sprintf("res/step1b_MAST_%s_glmer.tsv", ct1))
  
  p1p  = fread(sprintf("res/step1b_MAST_perm_%s_glm.tsv", ct1))
  p2p  = fread(sprintf("res/step1b_MAST_perm_%s_glmer.tsv", ct1))
  ...
  stopifnot(all(p1$V1 == p2p$V1))
  
  # ------------------------------------------------------------------------
  # read in gene expression data to check sparsity per gene
  # ------------------------------------------------------------------------
  
  dat1 = readRDS(file.path(data.dir, sprintf("ct_mtx/%s.rds", ct1)))
  dim(dat1)
  class(dat1)
  dat1[1:2,1:2]
  stopifnot(all(p1$V1 == rownames(dat1)))
  
  n.zeros = rowSums(dat1 == 0)
  summary(n.zeros)
  prop.zeros = n.zeros/ncol(dat1)
  summary(prop.zeros)
  
  summary(p1$V2[which(prop.zeros > 0.999)])
  summary(p2$V2[which(prop.zeros > 0.999)])
  summary(p1p$V2[which(prop.zeros > 0.999)])
  summary(p2p$V2[which(prop.zeros > 0.999)])
