...
for(k in 1:length(cell_types)){
  grp = cell_types[k]
  mast_glm   = fread(sprintf("res/1b_MAST_perm_%s_glm.tsv", grp))
  mast_glmer = fread(sprintf("res/1b_MAST_perm_%s_glmer.tsv", grp))
  rank_sum   = fread(sprintf("res/1b_ranksum_perm_%s.tsv", grp))
  saver      = fread(sprintf("res/1d_saver_direct_perm_pvals_%s.tsv", grp))
