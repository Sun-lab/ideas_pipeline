# compared with 2e, this version deals with the results when 
...
for(i in 1:length(cell_types)){
  ct1 = cell_types[i]
  q1  = fread(sprintf("res/3l_qvals_%s.tsv", ct1))
...
file.name = sprintf("res/4e_counts_sig_qval_threshold_%s.tsv",
                    as.character(round(threshold, digits = 2)))
fwrite(sig_count_df, file = file.name, sep = "\t")


pdf(sprintf("figures/4e_odds_ratio_qvalue_threshold_%f.pdf", 
...
pdf(sprintf("figures/4e_fisher_exact_neglogpvals_qvalue_threshold_%f.pdf", 
            round(threshold, digits = 2)), 
    width=6, height=6)
par(mfrow=c(1,1), bty="n", mar=c(5,4,3,1))
...
for(ct1 in cell_types){
  filename_n_overlap = sprintf("res/4e_n_overlap_%s_%f.csv", 
                               ct1, threshold)
  write.csv(n_overlap_list[[ct1]], file = filename_n_overlap, 
            row.names = TRUE)
  filename_prop_overlap = sprintf("res/4e_prop_overlap_%s_%f.csv", 
                                  ct1, threshold)
