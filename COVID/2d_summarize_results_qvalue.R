...
pdf("figures/2d_odds_ratio_qvalue_threshold.pdf", width=6, height=6)
...
pdf("figures/2d_fisher_exact_neglogpvals_qvalue_threshold.pdf", width=6, height=6)
par(mfrow=c(1,1), bty="n", mar=c(5,4,3,1))
...
for(ct1 in cell_types){
  filename_n_overlap = sprintf("res/2d_n_overlap_%s.csv", ct1)
  write.csv(n_overlap_list[[ct1]], file = filename_n_overlap, 
            row.names = TRUE, sep = ",")
  filename_prop_overlap = sprintf("res/2d_prop_overlap_%s.csv", ct1)
