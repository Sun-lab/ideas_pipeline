# compared to 3d, this version increases the time of permuation
# to 49999 and 99999


# this version is modified from step3d
...
# -rw-r--r--@ 1 1001593617 Oct  1 19:54 CD8+Tcells_1_dispersion.tsv
# -rw-r--r--  1 2032662952 Oct  1 19:46 CD8+Tcells_1_mean_norm.tsv
# -rw-r--r--@ 1  994056323 Oct  1 19:50 CD8+Tcells_1_pi.tsv
...
# subset from DCA results matrices

dca_par_list_perm = list(dca_mean[, cell2kp_perm], 
                         dca_disp[, cell2kp_perm], 
                         dca_pi[, cell2kp_perm])

table(colnames(dca_mean[, cell2kp_perm]) == meta2kp_perm$cell_id)
table(colnames(dca_disp[, cell2kp_perm]) == meta2kp_perm$cell_id)
table(colnames(dca_pi[, cell2kp_perm]) == meta2kp_perm$cell_id)
...
for(fit_method in c("dca_direct")){
  for(d_metric in c("Was", "JSD")){
    ...
    dist1 = ideas_dist(dca_par_list_perm, meta2kp_perm, meta_ind_perm, 
...
fig.name  = sprintf("figures/5d_dca_direct_pval_hist_%s_perm.pdf", 
                    grp)
file.name = sprintf("res/5d_dca_direct_pvals_%s_perm.tsv", 
                    grp)
