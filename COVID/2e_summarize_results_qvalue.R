...
for(i in 1:length(cell_types)){
  ct1 = cell_types[i]
  q1  = fread(sprintf("res/1l_qvals_%s.tsv", ct1))
...
file.name = sprintf("res/2e_counts_sig_qval_threshold_%s.tsv",
                    as.character(threshold))
fwrite(sig_count_df, file = file.name, sep = "\t")


pdf(sprintf("figures/2e_odds_ratio_qvalue_threshold_%f.pdf", threshold), 
    width=6, height=6)
...
pdf(sprintf("figures/2e_fisher_exact_neglogpvals_qvalue_threshold_%f.pdf", threshold), 
    width=6, height=6)
par(mfrow=c(1,1), bty="n", mar=c(5,4,3,1))
...
for(ct1 in cell_types){
  filename_n_overlap = sprintf("res/2e_n_overlap_%s_%f.csv", 
                               ct1, threshold)
  write.csv(n_overlap_list[[ct1]], file = filename_n_overlap, 
            row.names = TRUE)
  filename_prop_overlap = sprintf("res/2e_prop_overlap_%s_%f.csv", 
                                  ct1, threshold)
