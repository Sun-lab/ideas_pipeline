...
ranksum_pval=matrix(ncol=1,nrow=nrow(dat1))
for(iperm in 1:1){
  diagnosis=data.frame(perm_table)[,iperm] #nas can be ignored
  ranksum_pval[,iperm]=apply(dat1,1,function(x) wilcox.test(x[diagnosis=="mild"],x[diagnosis=="severe"])$p.value)
  print(iperm)
}
...
pdf(sprintf("figures/1b_ranksum_perm_%s_pval_hist_final.pdf", grp), 
    width=3, height=3)
par(mfrow=c(1,1), bty="n", mar=c(5,4,2,1))
hist(ranksum_pval, main="RankSum Test", xlab="p-value", breaks=20)
dev.off()
...
fwrite(data.frame(ranksum_pval), file=sprintf("res/1b_ranksum_perm_%s.tsv", grp), 
