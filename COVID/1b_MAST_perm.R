...
pdf(sprintf("figures/1b_MAST_perm_%s_compare_pval_Seq.pdf", grp), 
    width=9, height=3)
...
fwrite(data.frame(mast_pval_glm), 
       file=sprintf("res/1b_MAST_perm_%s_glm.tsv", grp), 
       sep = "\t", row.names=TRUE, col.names=FALSE)

fwrite(data.frame(mast_pval_glmer), 
       file=sprintf("res/1b_MAST_perm_%s_glmer.tsv", grp), 
            sep = "\t", row.names=TRUE, col.names=FALSE)
