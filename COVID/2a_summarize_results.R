...
for(ct1 in cell_types){
  ...
  gsea = readRDS(sprintf("res/1f_gsea_%s.rds", ct1))
  gsea = gsea[methods]
...
plot_gsea <- function(gseaL, ct1){
  ...
  pdf(sprintf("figures/2_GSEA_p_val_compare_%s.pdf", ct1), 
      width=6, height=3)
...
for (i in 1:length(methods)){
  ...
  if (length(row_sign_padj)>0){
    ...
    cur_figurename = 
      paste("figures/2_GSEA_CD8+Tcells_1_", m, ".pdf", sep = "")
...
pdf("figures/2_compare_pval_DESeq2_vs_IDEAS.pdf", width=4.5, height=4.5)
